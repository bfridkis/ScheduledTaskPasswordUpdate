...
#Get Remoting Credential (if non-default) and Credential for Task Scheduler Updates:
Do {
    If (!$localMachineOnly) {
        ...
        #If needed, get non-default (caller) credential...
        If($defaultCredential -eq "N") {
        ...
        $TaskCredentialPromptMessage =  "Input username and new password for scheduled task(s) update. " +
                                        ...
                                        "a user who is a member of a domain different than that of the user " +
                                        "who is calling this script, a domain qualifier can be used." +
...
#Determines current domain, needed in some cases for qualifying the username for scheduled tasks. 
##First method requires AD PS Module installation. See here: https://www.varonis.com/blog/powershell-active-directory-module
Try { 
    $domainObject = Get-ADDomain -Current LoggedOnUser -ErrorAction SilentlyContinue 
    If ($domainObject) { $currentDomain = $domainObject.Name }
} 
Catch {
    Try { $domainString = systeminfo | findstr /B "Domain"
          $currentDomain = $domainString.split(":").trim().split(".")[1]
    }
    Catch { }
}
...
If (!$localMachineOnly) {
    ...
    $comps | ForEach-Object { 
        $thisComp = $_
        If(!$defaultCredential -eq "N") { $thisResult = Invoke-Command -ComputerName $thisComp -Credential $Credential -ScriptBlock $scriptblock1 -ArgumentList $TaskCredential, $currentDomain -ErrorVariable errmsg 2>$null }
        Else { $thisResult = Invoke-Command -ComputerName $thisComp -ScriptBlock $scriptblock1 -ArgumentList $TaskCredential, $currentDomain -ErrorVariable errmsg 2>$null }
...
Else {
    ...
    If(!$results -and !$errors) { $errors.Add( [PSCustomObject]@{ 'Hostname' = $ENV:Computername; 'Exception' = "No tasks found for '$($TaskCredential.Username)' on localhost. (Try running as admin.)" } ) } 
}
